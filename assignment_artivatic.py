...
Hence, as a workaround if we try to impute this 85%+ missing data with the same as corresponding title data, 
it will be a situation similar to multicolinearity where two columns would almopst be around 90% similar, hence
...
We are doing this for train & test data.
...
Now not going for frequency here this is more stablke technique, and where we have huge no of categories 
...
from xgboost import XGBClassifier
dec = XGBClassifier()
parameters = {'max_depth':[1, 2, 3], 'n_estimators':[10, 20, 30], 'learning_rate':[0.1, 0.2]}  
clf = GridSearchCV(dec, parameters, cv=3, scoring='roc_auc', return_train_score=True) 
clf.fit(X_train_scaled, y_train) 

results = pd.DataFrame.from_dict(clf.cv_results_)
results1 = results.sort_values(['param_n_estimators'])
results2 = results.sort_values(['param_max_depth'])
end = time.time()
...
"""Taking all values to plot in variables"""

train_auc= results['mean_train_score'] 

cv_auc = results['mean_test_score'] 

est_val =  results1['param_n_estimators']
dep_val = results2['param_max_depth']

"""importing plotly and other packages for plotting"""
...
trace1 = go.Scatter3d(x=est_val,y=dep_val,z=train_auc, name = 'train')
trace2 = go.Scatter3d(x=est_val,y=dep_val,z=cv_auc, name = 'Cross validation')
data = [trace1, trace2]
...
dec = XGBClassifier(max_depth=depth_best, n_estimators=est_best, lea_best = 0.2)
...
Here curve blue is not visible as they both are overlapping
"""
...
df_predicted_finally_test_labels = pd.DataFrame(columns=['member_id', 'loan_status', 'prob_values'])
df_predicted_finally_test_labels['member_id'] = X_Unseen_test['member_id']
df_predicted_finally_test_labels['loan_status'] = y_unseen_pred_loan_status
df_predicted_finally_test_labels['prob_values'] = y_unseen_pred

"""Hence, below we see that from test set given 70.82% will be marked as 1(defaulter) by model"""

print(df_predicted_finally_test_labels.shape)
df_predicted_finally_test_labels['loan_status'].value_counts()

"""Saving results to a file"""

df_predicted_finally_test.to_csv('result_submission.csv')
